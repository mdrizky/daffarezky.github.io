...
const calcFollowers = document.getElementById('calcFollowers');
const calcEng = document.getElementById('calcEng');
const resEng = document.getElementById('resEng');
const rateEng = document.getElementById('rateEng');
...
const rateRoi = document.getElementById('rateRoi');

function updateEng() {
    const f = parseFloat(calcFollowers.value) || 0;
    const e = parseFloat(calcEng.value) || 0;
    if (f === 0) { resEng.textContent = '0%'; return; }
    const rate = (e / f) * 100;
    resEng.textContent = rate.toFixed(1) + '%';
    
    rateEng.className = 'result-rating';
    if (rate >= 5) { rateEng.classList.add('elite'); rateEng.textContent = 'Elite'; }
    else if (rate >= 2) { rateEng.classList.add('good'); rateEng.textContent = 'Good'; }
    else { rateEng.classList.add('poor'); rateEng.textContent = 'Poor'; }
}

function updateRoi() {
    const s = parseFloat(calcSpend.value) || 0;
    const r = parseFloat(calcRevenue.value) || 0;
    if (s === 0) { resRoi.textContent = '0%'; return; }
    const roi = ((r - s) / s) * 100;
    resRoi.textContent = roi.toFixed(0) + '%';
    
    rateRoi.className = 'result-rating';
    if (roi >= 200) { rateRoi.classList.add('elite'); rateRoi.textContent = 'Highly Profitable'; }
    else if (roi > 0) { rateRoi.classList.add('good'); rateRoi.textContent = 'Profitable'; }
    else { rateRoi.classList.add('poor'); rateRoi.textContent = 'Loss'; }
}

if (calcFollowers) {
    calcFollowers.addEventListener('input', updateEng);
    calcEng.addEventListener('input', updateEng);
}
if (calcSpend) {
    calcSpend.addEventListener('input', updateRoi);
    calcRevenue.addEventListener('input', updateRoi);
}

// ─── ADD NEW TRANSLATIONS ───
const extraTranslations = {
    en: {
        tools_tag: "Strategic Tools",
        tools_title: "Data doesn't lie.<br>Analyze like a pro.",
        tools_desc: "Use these calculators to measure business performance instantly.",
        calc_eng_title: "Engagement Rate",
        label_followers: "Followers",
        label_eng_actions: "Total Engagement (Likes + Comments)",
        label_eng_res: "Current Engagement Rate",
        calc_roi_title: "ROI Simulator",
        label_spend: "Ad Spend ($)",
        label_revenue: "Revenue ($)",
        label_roi_res: "Return on Investment",
        projects_tag: "Featured Projects",
        projects_title: "Selected Case Studies.",
        projects_desc: "Strategic work showing end-to-end execution.",
        view_case: "View Case Study",
        proj_kopi_desc: "Positioning, content funnel, and CTA strategy for students. Boosting engagement via storytelling.",
        proj_kpi_desc: "CTR, conversion, and weekly insight simulation. Tracking real-time business performance.",
        proj_market_desc: "Urgency, social proof, and checkout optimization analysis for improved conversion rates."
    },
    id: {
        tools_tag: "Alat Strategis",
        tools_title: "Data Tidak Berbohong.<br>Analisa Layaknya Pro.",
        tools_desc: "Gunakan kalkulator ini untuk mengukur performa bisnis secara instan.",
        calc_eng_title: "Tingkat Engagement",
        label_followers: "Pengikut",
        label_eng_actions: "Total Engagement (Likes + Komen)",
        label_eng_res: "Tingkat Engagement Saat Ini",
        calc_roi_title: "Simulator ROI",
        label_spend: "Biaya Iklan ($)",
        label_revenue: "Pendapatan ($)",
        label_roi_res: "Pengembalian Investasi (ROI)",
        projects_tag: "Projek Unggulan",
        projects_title: "Studi Kasus Terpilih.",
        projects_desc: "Pekerjaan strategis yang menunjukkan eksekusi dari hulu ke hilir.",
        view_case: "Lihat Projek",
        proj_kopi_desc: "Positioning, content funnel, dan strategi CTA untuk mahasiswa. Meningkatkan engagement lewat dongeng bisnis.",
        proj_kpi_desc: "Simulasi CTR, konversi, dan insight mingguan. Melacak performa bisnis secara real-time.",
        proj_market_desc: "Analisa urgensi, bukti sosial, dan optimasi checkout untuk meningkatkan tingkat konversi."
    }
};
