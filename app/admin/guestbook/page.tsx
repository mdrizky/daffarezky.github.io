    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        <h1 className="text-3xl font-bold font-syne text-gray-900 dark:text-white flex items-center gap-3">
          <FaBook className="text-blue-500" /> Kelola Guestbook
        </h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1">Moderasi pesan yang masuk di halaman Guestbook.</p>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-white/5 border-bottom border-gray-200 dark:border-white/10">
                <th className="px-6 py-4 text-sm font-bold text-gray-700 dark:text-gray-300">Nama</th>
                <th className="px-6 py-4 text-sm font-bold text-gray-700 dark:text-gray-300">Pesan</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
              {entries.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-10 text-center text-gray-500">Belum ada entry guestbook.</td>
                </tr>
              ) : (
                entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right space-x-2">
                      <button
                        ...
                        title={entry.is_approved ? 'Batalkan Approval' : 'Setujui'}
                      >
                        {entry.is_approved ? <FaTimes /> : <FaCheck />}
                      </button>
                      <button
